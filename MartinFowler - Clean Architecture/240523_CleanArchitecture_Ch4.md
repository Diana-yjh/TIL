...
위 1번 그림에서 X는 __안정된 컴포넌트__ 입니다.</br>
세 개의 컴포넌트가 X에 의존하며 따라서 X는 변경이 지양되기 때문입니다.</br>

반대로 2번 그림의 Y는 변경될 수 있는 요인이 3 가지나 되기 때문에 __안정되지 않은 컴포넌트__ 입니다.</br>

그럼 안정성 지표를 확인하는 방법은 무엇이 있을까요?</br>
이 방법으로는 컴포넌트로 들어오고 나가는 의존성 개수를 세어보는 방법이 있습니다.</br>
- `Fan-in`: 안으로 들어오는 의존성
- `Fan-out`: 바깥으로 나가는 의존성
- `I(불안정성)`: `I` = `Fan-out`/ (`Fan-in`+`Fan-out`) , `I`는 0 일 때 최고로 안정된 상태, 1 일 때는 불안정한 상태이다.</br>

그렇다고 모든 컴포넌트가 반드시 안정적이어야 하는 것은 아닙니다.</br>
...
이처럼 SDP가 위배되는 상황도 확인할 수 있습니다.</br>
이 문제를 해결하기 위해서는 우리는 DIP를 사용할 수 있습니다.</br>
...
위 그림에서 `UServer`를 우리는 __추상 컴포넌트__ 라고 부릅니다.</br>

#### 4. SAP: 안정된 추상화 원칙
__컴포넌트는 안정된 정도 만큼만 추상화 되어야 한다.__ </br>

우리는 회사 정책과 같은 업무 로직의 변경을 달가워하지 않습니다.</br>
따라서 이런 부분들을 안정된 컴포넌트에 위치시키고자 하죠.</br>
하지만 이러면 정책을 포함한 코드들은 수정이 어려워지는 문제가 발생합니다.</br>

이런 사면초가의 문제의 해결책으로 우리는 OCP를 활용할 수 있습니다.</br>
클래스의 수정 없이 확장이 가능할 정도로 클래스를 유연하게 만들 수 있는 __추상클래스__ 를 사용하면 됩니다.</br>

SAP는 안정성과 추상화 정도 사이의 관계를 정의합니다.</br>
여기서 우리는 안정된 컴포넌트는 추상 클래스로, 그리고 불안정한 컴포넌트는 구체 컴포넌트로 생성할 것을 추천하죠.</br>

SAP와 SDP를 결합하면 컴포넌트에 대한 DIP와 유사하게 되는데 결국 __의존성은 추상화 방향으로만 흐를 것__ 을 이야기 합니다.</br>

### 4. 참고 자료
CleanArchitecture - 4부
